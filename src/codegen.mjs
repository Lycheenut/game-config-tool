...
export async function generateConfigCodeFromDirectory(options = {}) {
    ...
    const outputPath = resolveOutputPath(outputRoot, outputFile);
    const payload = await readConfigSnapshot(inputRoot);
    const code = generateConfigCode(payload, options);
...
export function generateConfigCode(payload, options = {}) {
    ...
    const names = createNameRegistry(schema, tables, constants, options);
    const chunks = [
        '/* eslint-disable */',
    ...
    chunks.push(...generateEnumDefinitions(schema, names));
    chunks.push(...generateStructureDefinitions(schema, names));
    chunks.push(...generateTableDefinitions(tables, names));
    chunks.push(...generateConstantDefinitions(constants, names));
    chunks.push(...generateRepository(tables, constants, names, options));
    chunks.push(...runtimeHelpers());
...
function generateTableDefinitions(tables, names) {
    const lines = [];
...
function generateRepository(tables, constants, names, options) {
    ...
    for (const table of tables) {
        ...
        lines.push(`    private ${property}Rows: ${rowType}[] = [];`);
        lines.push(`    private ${property}ById = new Map<number, ${rowType}>();`);
    }
    for (const constant of constants) {
    ...
    if (tables.length + constants.length === 0) {
        ...
        for (const table of tables) {
            lines.push(`            loadCsv(fetchImpl, joinConfigUrl(baseUrl, ${JSON.stringify(table.path)}), parse${names.tableTypes.get(table.moduleKey)}),`);
        }
        for (const constant of constants) {
            lines.push(`            loadJson<${names.constantTypes.get(constant.moduleKey)}>(fetchImpl, joinConfigUrl(baseUrl, ${JSON.stringify(constant.path)})),`);
        }
        lines.push('        ]);');
        for (const table of tables) {
            const property = names.tableProperties.get(table.moduleKey);
            lines.push(`        this.${property}Rows = ${property}Rows;`);
            lines.push(`        this.${property}ById = new Map(${property}Rows.map((row) => [row.id, row]));`);
        }
        for (const constant of constants) {
            const property = names.constantProperties.get(constant.moduleKey);
    ...
    for (const table of tables) {
        ...
        lines.push(`        const value = this.get${method}ById(id);`);
        lines.push('        if (!value) {');
        lines.push(`            throw new Error(${JSON.stringify(`${table.moduleKey} config row not found: `)} + id);`);
        lines.push('        }');
    ...
    for (const constant of constants) {
        ...
        lines.push(`        return requireLoaded(this.${property}Value, ${JSON.stringify(constant.moduleKey)});`);
        lines.push('    }');
    }
...
function runtimeHelpers() {
    return [
        ...
        '}',
        '',
        'async function loadJson<T>(fetchImpl: FetchLike, url: string): Promise<T> {',
        '    const response = await fetchImpl(url);',
        '    if (!response.ok) {',
        '        throw new Error(`Unable to load ${url}: ${response.status}`);',
        '    }',
        ...
        '}',
        '',
        'function joinConfigUrl(baseUrl: string, configPath: string): string {',
        ...
        '}',
        '',
        'function numberArrayValue(value: string | undefined): number[] {',
        '    return parseArrayValue<number>(value, \'number\');',
        '}',
        '',
        'function booleanArrayValue(value: string | undefined): boolean[] {',
        ...
        '}',
        '',
        'function jsonValue<T>(value: string | undefined, fallback: unknown): T {',
        '    if (value === undefined || value === \'\') {',
        ...
        '}',
        '',
        'function requireLoaded<T>(value: T | undefined, label: string): T {',
        '    if (value === undefined) {',
...
function createNameRegistry(schema, tables, constants, options) {
    ...
    for (const table of tables) {
        ...
        tableTypes.set(table.moduleKey, uniqueName(usedTypes, typeName));
    ...
    return {
        repositoryName: uniqueName(usedTypes, pascalCase(options.repositoryName || 'GeneratedConfigRepository', 'GeneratedConfigRepository')),
        enumTypes,
        structureTypes,
        tableTypes,
        constantTypes,
